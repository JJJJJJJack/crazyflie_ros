...
private:
  ...
  {
    //ROS_INFO_THROTTLE(10, "Update parameters");
  ...
  void run()
  {
          ...
          || m_enable_logging_battery)
	{
      ...
      // Add custom logging for ANDO
      if (m_enable_logging_ando)
      {
        std::function<void(uint32_t, logDHAT*)> cbdhat         = std::bind(&CrazyflieROS::onLogDHATData, this, std::placeholders::_1, std::placeholders::_2);
        std::function<void(uint32_t, logCompensatePWM*)> cbpwm = std::bind(&CrazyflieROS::onLogCompensatePWMData, this, std::placeholders::_1, std::placeholders::_2);
        std::function<void(uint32_t, logMOTOR*)> cbmotor       = std::bind(&CrazyflieROS::onLogMOTORData, this, std::placeholders::_1, std::placeholders::_2);

        logBlockDHAT.reset(new LogBlock<logDHAT>(
          &m_cf,{
            {"ANDO", "dhatx"},
            {"ANDO", "dhaty"},
            {"ANDO", "dhatz"},
        ...
        logBlockDHAT->start(2); // 20ms 50Hz
        logBlockCompensatePWM.reset(new LogBlock<logCompensatePWM>(
          &m_cf,{
            {"ANDO", "pwm_roll"},
            {"ANDO", "pwm_pitch"},
            {"ANDO", "pwm_yaw"},
        ...
        logBlockCompensatePWM->start(2); // 20ms 50Hz
  ...
  // Jack logging additional ando data
  void onLogDHATData(uint32_t time_in_ms, logDHAT* data) {
    if (m_enable_logging_ando) {
      m_dhat.x = data->dhatx;
      m_dhat.y = data->dhaty;
      m_dhat.z = data->dhatz;
  ...
  void onLogCompensatePWMData(uint32_t time_in_ms, logCompensatePWM* data) {
    if (m_enable_logging_ando) {
      crazyflie_driver::ANDO msg;
      if (m_use_ros_time) {
	msg.header.stamp = ros::Time::now();
      } else {
        msg.header.stamp = ros::Time(time_in_ms / 1000.0);
      ...
      m_pubANDO.publish(msg);
    }
  }

  void onLogMOTORData(uint32_t time_in_ms, logMOTOR* data) {
    if (m_enable_logging_ando) {
      m_motor.M1 = data->M1;
      m_motor.M2 = data->M2;
      m_motor.M3 = data->M3;
...
private:
  ...
  float m_roll_trim;
  float m_pitch_trim;
  geometry_msgs::Quaternion m_dhat;
  logMOTOR m_motor;
